...
# RTrain_X = RTrain_X.drop('number', axis=1).copy()
# reg_nan = reg_nan.drop('method', axis = 1).copy()
# reg_nan = reg_nan.drop('year', axis = 1).copy()
# reg_nan = reg_nan.drop('number', axis = 1).copy()
...
# Knn_Reg1 = Pipeline([("STD", StandardScaler()),
...
Knn_Reg2 = Pipeline([("STD", StandardScaler()),
("Knn", KNeighborsRegressor(weights ='distance', n_neighbors=3)) #######################################################################3
])
...
train['mass'].fillna(0, inplace=True)
...
# #reg_nant = reg_nant.drop("number", axis=1).copy()
...
reg_test2 = test[test['distance'].isnull() == False].copy()
reg_nan2_test = test[test['distance'].isnull() == True].copy()
Rtest_X2 = reg_test2.drop("distance", axis=1).copy() # test X
Rtest_y2 = reg_test2['distance'].copy() # test y
reg_nan2_test = reg_nan2_test.drop('distance', axis=1).copy()

Rtest_X2_array = imputer.fit_transform(Rtest_X2)
reg_nan2_test_array = imputer.fit_transform(reg_nan2_test)
Rtest_X2 = pd.DataFrame(Rtest_X2_array, index=Rtest_X2.index, columns=Rtest_X2.columns).copy()
reg_nan2_test = pd.DataFrame(reg_nan2_test_array, index=reg_nan2_test.index, columns=reg_nan2_test.columns).copy()

Rtest_X2 = Rtest_X2.drop('number', axis=1).copy()
Rtest_X2 = Rtest_X2.drop('orbital_period', axis=1).copy()
reg_nan2_test = reg_nan2_test.drop('number', axis = 1).copy()

reg_nan2_test = reg_nan2_test.drop('orbital_period', axis = 1).copy()


Knn_Reg2.fit(Rtest_X2, Rtest_y2)

# predict and Trans ndarray to Series
predict_distance = Knn_Reg2.predict(reg_nan2_test)
predict_distance = pd.Series((predict_distance), name='distance').copy()
Rtest_y2.index = Rtest_X2.index
predict_distance.index = reg_nan2_test.index
test_distance = pd.concat([predict_distance, Rtest_y2], axis = 0).copy()
test_distance = test_distance.sort_index().copy()
...
# orbital_period NaN Processing
test['mass'].fillna(0, inplace=True)
...
pred_method_df.to_csv("r1.csv", index=True)
