...
SGD_Reg = KNeighborsRegressor(n_neighbors=3, weights='distance')
SGD_Reg.fit(RTrain_X, RTrain_y)

# predict and Trans ndarray to Series
predict_mass = SGD_Reg.predict(reg_nan)
...
# Column Settinng
train = train[['method', 'orbital_period', 'mass', 'distance', 'year']]
...
## 2022-05-04 여기서부터 다시 model selection
ppl = Pipeline([
    ("std", StandardScaler()),
...
pred_method_df.to_csv("Knn_09.csv", index=True)
