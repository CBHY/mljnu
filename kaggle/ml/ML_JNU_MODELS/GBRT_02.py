...
ppl = Pipeline([
    ("gbrt", GradientBoostingClassifier(max_depth = 3, n_estimators= 10, learning_rate= 0.3 ,random_state = 42))
])
...
pred_method_df = pd.DataFrame({'index': test['index'], 'method': pred_method})


pred_method_df.to_csv("model 02_hy.csv", index=False)
